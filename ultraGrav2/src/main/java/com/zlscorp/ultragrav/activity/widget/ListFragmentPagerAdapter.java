...
public class ListFragmentPagerAdapter extends FragmentPagerAdapter implements
	...
	public Fragment getItem(int position) {
		...
		Fragment frag = Fragment.instantiate(context, item.fragmentClass,
				item.args);
	...
	public void onPageScrolled(int position, float positionOffset,
			int positionOffsetPixels) {/* ignore */
	}
	...
	private void selectFragment(int position) {
		...
		if (currentRef != null && currentRef.get() != null) {
			...
			if (unselected instanceof OnFragmentSelectedListener) {
				((OnFragmentSelectedListener) unselected)
